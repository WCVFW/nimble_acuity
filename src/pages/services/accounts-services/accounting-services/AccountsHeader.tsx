    ...
    <nav className="fixed w-full bg-white shadow-md z-20 px-4 md:px-6 py-4">
      <div className="flex items-center justify-between max-w-7xl mx-auto">
        ...
        <div className="md:hidden">
          ...
          {isMobileMenuOpen && (
            <div className="absolute right-4 mt-2 w-56 bg-white border border-gray-200 rounded-md shadow-lg py-1">
        ...
        {/* Desktop Navigation with Extra Gap */}
        <ul className="hidden md:flex space-x-10"> {/* ⬅️ increased gap */}
          {megaMenuData.map((item, index) => {
            ...
            return (
              <li
                key={index}
                className="relative group cursor-pointer hover:text-[#006A7C] transition-colors duration-200"
              >
                <a
                  href={item.href}
